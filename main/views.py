...
def verify_subscription_payment(request):
    ...
    return Response({'message': 'Subscription verified.'}, status=200)import requests
from rest_framework.viewsets import ModelViewSet, ReadOnlyModelViewSet
from rest_framework.exceptions import PermissionDenied
from rest_framework.decorators import api_view, permission_classes, action
from rest_framework.permissions import IsAuthenticatedOrReadOnly, IsAuthenticated
...
class MessageViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        qs = models.Message.objects.filter(Q(sender=user) | Q(recipient=user))
        other_id = self.request.query_params.get('user_id')
        if other_id:
            qs = qs.filter(
                (Q(sender=user, recipient_id=other_id) |
    ...
    def list(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(queryset, many=True)
        return Response({'current_user': request.user.id, 'messages': serializer.data})
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Build a new payload dict instead of mutating request.data
        payload = {**request.data, "recipient": recipient_id}

        # Run through the normal serializer create flow
    ...
    def perform_create(self, serializer):
        ...
        tokens = models.PushToken.objects.filter(user=msg.recipient).values_list('token', flat=True)
        title = f"New message from {self.request.user.username}"
        body  = msg.content[:100]
        for t in tokens:
            try:
                send_expo_push(t, title, body, data={'chatId': str(msg.id)})
            except Exception:
                pass  # optionally log
        return msg
    ...
    def perform_update(self, serializer):
        ...
        user = self.request.user
        if inst.sender != user and inst.recipient != user:
            raise PermissionDenied("You cannot update this message.")
...
class NotificationViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # --- SEND PUSH to user ---
        tokens = models.PushToken.objects.filter(user=note.user).values_list('token', flat=True)
        title = "You have a new notification"
        body  = dict(models.Notification.TYPE_CHOICES)[note.notif_type]
        for t in tokens:
            try:
                send_expo_push(t, title, body, data={'notifId': str(note.id)})
            except Exception:
                pass
        return note
...
class PushTokenViewSet(ModelViewSet):

    serializer_class = serializers.PushTokenSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        return models.PushToken.objects.filter(user=self.request.user)

    def perform_create(self, serializer):

        token = serializer.validated_data['token']
        obj, _ = models.PushToken.objects.update_or_create(
    ...
    def remove(self, request):
        ...
        if token:
            models.PushToken.objects.filter(user=request.user, token=token).delete()
            return Response(status=status.HTTP_204_NO_CONTENT)
